...
class GatewayRPCHandler:
    ...
    async def handle(self, topic: str, payload: bytes) -> Optional[GatewayRPCResponse]:
        ...
        try:
            ...
            if device_session:
                rpc_request.set_device_session(device_session)
            else:
            ...
            result = await self._event_dispatcher.dispatch(rpc_request)
            if not isinstance(result, GatewayRPCResponse):
                logger.error("RPC callback must return an instance of GatewayRPCResponse, got: %s", type(result))
                return None
            ...
            if rpc_request is None:
                return None
            GatewayRPCResponse.build(device_name=rpc_request.device_name, request_id=rpc_request.request_id, error=e)
