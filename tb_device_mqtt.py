...
class TBClient:
    ...
    def __init__(self, host, token):
        ...
        def on_disconnect(client, userdata, rc):
            ...
            if rc == 0:
                ...
                log.error("disconnect FAIL with error code {rc}".format(rc=rc))
    ...
    def subscribe(self, key="*", callback=None, quality_of_service=1):
        ...
        if key == "*":
            for attr in self.__sub_dict.keys():
                if inst not in self.__sub_dict[attr]:
                    ...
                    log.debug("Subscribed to {attr} subscription id {sub_id}".format(attr=attr,
                                                                                     sub_id=inst.subscription_id))
        # if attribute doesn't exist create it with subscription
            ...
            log.debug("Subscribed to {attr} subscription id {sub_id}".format(attr=key,
                                                                             sub_id=inst.subscription_id))
